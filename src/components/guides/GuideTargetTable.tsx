  ...
  return (
    <section className="guide-target-section" aria-labelledby="guide-target-title">
      <div className="guide-section-heading">
        ...
        <h2 id="guide-target-title">観測対象</h2>
      </div>
        ...
        {targets.map((target) => (
          <article className="guide-target-row" key={target.starId}>
            <div className="guide-target-details">
              ...
              <strong>{target.nameJa ?? target.name}</strong>
              {target.nameJa && <span className="guide-target-en">{target.name}</span>}
            ...
            <div className="guide-check-options" aria-label={`${target.name} の観測結果`}>
