...
const reference = cloudSnapshotReferenceFromRecord(record, mission.id, "user-1");
check("CLOUD-SNAPSHOT-1: reference is valid", isCloudMissionSnapshotReference(reference));
check("CLOUD-SNAPSHOT-1: path is scoped by user, Mission, and Snapshot", reference.storagePath === "user-1/mission-cloud-snapshot/snapshot-cloud-1.png");
...
try { cloudSnapshotReferenceFromRecord({ ...record, mimeType: "image/jpeg", blob: new Blob(["jpg"], { type: "image/jpeg" }) } as unknown as SkySnapshotRecord, mission.id, "user-1"); } catch { rejectedType = true; }
check("CLOUD-SNAPSHOT-5: non-PNG assets are rejected", rejectedType);
let rejectedSize = false;
try { cloudSnapshotReferenceFromRecord({ ...record, blob: new Blob([new Uint8Array(MAX_SKY_SNAPSHOT_BYTES + 1)], { type: "image/png" }) }, mission.id, "user-1"); } catch { rejectedSize = true; }
